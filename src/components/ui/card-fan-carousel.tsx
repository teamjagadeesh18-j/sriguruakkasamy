...
const ARROW_CLASSES =
  "relative flex items-center justify-center rounded-full border-[1.5px] border-black/10  bg-black/5  backdrop-blur-[16px] text-black/60  cursor-pointer shrink-0 z-30 outline-none shadow-[0_4px_20px_rgba(0,0,0,0.1)]  hover:border-black/25  hover:text-black/90  active:opacity-70 transition-colors duration-300 before:content-[''] before:absolute before:inset-[3px] before:rounded-full before:border before:border-black/[0.04]  before:pointer-events-none";
...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  return (
    <div className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20 overflow-hidden min-h-[420px] md:min-h-[480px]">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-[340px] md:h-[400px]">
          {cards.map((card, index) => {
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card absolute w-[160px] h-[230px] sm:w-[190px] sm:h-[280px] md:w-[210px] md:h-[310px] lg:w-[240px] lg:h-[350px] rounded-2xl overflow-hidden shadow-2xl border border-slate-200/80 bg-white block cursor-pointer">{image}</a>
            ) : (
              <div key={index} className="fan-card absolute w-[160px] h-[230px] sm:w-[190px] sm:h-[280px] md:w-[210px] md:h-[310px] lg:w-[240px] lg:h-[350px] rounded-2xl overflow-hidden shadow-2xl border border-slate-200/80 bg-white">{image}</div>
            );
          })}
        </div>
      </div>
