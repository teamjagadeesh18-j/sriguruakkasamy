...
export function InfiniteRibbon({
  ...
  return (
    <div
      className={cn(
        "w-full max-w-full overflow-hidden py-4 text-sm sm:text-base md:text-lg font-semibold tracking-wide font-body select-none",
        className
      )}
    ...
    >
      ...
      >
        {Array.from({ length: repeatCount * 2 }, (_, index) => (
          <span className="mr-10 inline-flex items-center gap-6" key={index}>
            {children}
          </span>
        ))}
      </div>
